...
def main():
    ...
    features_cm_to_pixels_dict =  {(0, 0): (192, 801, ),
                                  (214.2, 0): (937, 652),
                                  (214.2, 178.5): (798, 67),
                                  (0, 178.5): (101, 197)}
    A = np.zeros((2 * len(features_cm_to_pixels_dict), 6), dtype=float)
    b = np.zeros((2 * len(features_cm_to_pixels_dict), 1), dtype=float)
    index = 0
    for XY, xy in features_cm_to_pixels_dict.items():
        X = XY[0]
        Y = XY[1]
        x = xy[0]
        y = xy[1]
    ...
    test_xy_1 = (399, 500, 1)
    test_XY_1 = pixels_to_cm_transformation_mtx @ test_xy_1
    ...
    test_xy_2 = cm_to_pixels_transformation_mtx @ test_XY_2
